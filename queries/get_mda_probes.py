...
from diamond_miner.queries.query import LinksQuery, links_table, probes_table, asprobes_table, results_view, group_mapping, links_view
    ...
    def statement(
        ...
        FROM (
            SELECT * from(
                ...
                link.near_ttl == results.probe_ttl AND
                link.probe_src_port == results.probe_src_port
                ) as tmp
                INNER JOIN {group_mapping(measurement_id)} gm USING ip_asn,asn
            )
        GROUP BY (probe_protocol, probe_src_addr, group_id)
        """
...
class InsertMDAProbes(GetMDAProbes):
    def statement(
        ...
        return f"""
        INSERT INTO {asprobes_table(measurement_id)}
        WITH
