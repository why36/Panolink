...
class GetLinksFromResults(ResultsQuery):
    ...
    def statement(
        ...
        return f"""
        WITH
            groupUniqArray((round, probe_ttl, reply_src_addr, asn)) AS traceroute,
            arrayMap(x -> x.2, traceroute) AS ttls,
            arrayMap(x -> (x.1, x.3, x.4), traceroute) AS val,
            CAST((ttls, val), 'Map(UInt8, Tuple(UInt8, IPv6, String))') AS map,
